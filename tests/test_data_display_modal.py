...
class TestDataDisplayModalAPI:
    ...
    def test_get_data_source_config(self):
        ...
        assert all("label" in option and "value" in option for option in config_data["dataSourceOptions"])
    ...
    def test_get_update_progress(self):
        """测试获取更新进度"""
        task_id = "update_20240115_120000"
        response = client.get(f"/api/data-sources/update/progress/{task_id}")
        assert response.status_code == 200
    ...
    def test_get_export_status(self):
        """测试获取导出状态"""
        task_id = "export_20240115_120000"
        response = client.get(f"/api/data-sources/export/status/{task_id}")
        assert response.status_code == 200
